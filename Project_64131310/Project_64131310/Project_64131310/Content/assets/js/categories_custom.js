...
jQuery(document).ready(function ($) {
	...
	*/
	...
	function DeleteWishlist(id) {
		$.ajax({
			url: '/Wishlist_64131310/PostDeleteWishlist',
			type: 'POST',
			data: { ProductId: id },
	...
	function AddWishlist(id) {
		$.ajax({
			url: '/Wishlist_64131310/postwishlist',
			type: 'POST',
			data: { ProductId: id },
			success: function (res) {
				if (res.Success == false) {
		...
		if ($('.product-grid').length) {
			...
			sortNums.each(function () {
				$(this).on('click', function () {
					...
					// Kiểm tra nếu lựa chọn là 'All'
					if (numSortingText === 'All') {
						$('.num_sorting_btn').text('Tất cả');
						$('.product-grid').isotope({ filter: '*' }); // Hiển thị tất cả các phần tử
					} else {
						var numFilter = ':nth-child(-n+' + numSortingText + ')';
						$('.num_sorting_btn').text(numSortingText);
